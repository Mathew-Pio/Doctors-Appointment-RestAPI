...
export const updateDoctor = async (req, res) => {
    ...
    try{
        let updatedDoctor;
        updatedDoctor = await Doctor.findByIdAndUpdate(id, 
            {$set: req.body}, 
            {new: true}
        )

        return res.status(200).json({success: true, message: 'Doctor successfully updated', data: updatedDoctor});
    }catch(err){
...
export const getAllDoctors = async (req, res) => {
    ...
    try{
        ...
        return res.status(500).json({success: false, message: 'Failed to find doctors'});
    }
}